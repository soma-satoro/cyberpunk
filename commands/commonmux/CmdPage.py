...
class CmdPage(DefaultCmdPage):
    ...
    @staticmethod
    def _parse_recipients(targets):
        """
        Parse recipient names separated by spaces/commas, with quote support.

        Examples:
            Nick,Jane -> ["Nick", "Jane"]
            "Nick Nick" Jane -> ["Nick Nick", "Jane"]
            "Nick Nick",Jane -> ["Nick Nick", "Jane"]
        """
        token_re = re.compile(r'"([^"]+)"|\'([^\']+)\'|([^,\s]+)')
        recipients = []
        for match in token_re.finditer(targets):
    ...
    @staticmethod
    def _is_staff_account(account):
        """
        Identify staff-like accounts so paging can bypass restrictive msg locks.
        """
        if not account:
            return False

        if getattr(account, "is_staff", False) or getattr(account, "is_superuser", False):
            return True

        try:
            return any(
                account.permissions.check(perm)
                for perm in ("Helper", "Builder", "Admin", "Developer")
            )
        except Exception:
            return False
    ...
    def func(self):
        ...
        # Split recipients by spaces/commas, while keeping quoted names intact.
        recipient_list = self._parse_recipients(targets)
        ...
        for target in account_recipients:
            # Account locks should be checked against the caller's account when possible.
            accessing_obj = caller.account if hasattr(caller, "account") and caller.account else caller
            can_msg = target.access(accessing_obj, "msg")
            if not can_msg and not self._is_staff_account(target):
