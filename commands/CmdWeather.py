...
class CmdWeather(default_cmds.MuxCommand):
    ...
    def get_tide_info(self):
        ...
        tide_url = "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?date=today&station=9412110&product=predictions&datum=STND&time_zone=lst_ldt&interval=hilo&units=english&format=json"
    ...
    def func(self):
        ...
        # IC: same Pacific calendar date and clock as the server, year advanced to 2076
        now = datetime.now(night_city_tz).replace(year=2076)
        current_date = now.strftime("%A, %B %d, %Y")
        current_time = now.strftime("%I:%M %p")

        width = 78
        output = []
        output.append(self.format_header("Night CityMUX Weather", width=width))
        ...
        if custom_weather:
            ...
            city_id = "5392323"  # San Luis Obispo city ID, roughly where Night City is.
