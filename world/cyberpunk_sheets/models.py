...
class CharacterSheet(SharedMemoryModel):
    ...
    languages = models.ManyToManyField('languages.Language', through='languages.CharacterLanguage', related_name='character_sheets')    
    class Meta:
    ...
    def add_language(self, language_name, level):
        ...
        char_lang, created = CharacterLanguage.objects.update_or_create(
    ...
    def clear_cultural_languages(self):
        # This assumes that languages with level 4 are from cultural origin
        self.character_languages.filter(level=4).delete()
    ...
    def add_language(self, language_name, level):
        ...
        if language_info:
            language, created = Language.objects.get_or_create(
                ...
                defaults={
                    'local': language_info.get('local', False),
                    'corporate': language_info.get('corporate', False)
                }
            )
        else:
            language, created = Language.objects.get_or_create(name=language_name)
    ...
    def get_note(self, name):
        return self.notes.filter(name__iexact=name).first()
    ...
    def get_notes_by_category(self, category):
        return self.notes.filter(category__iexact=category)
    ...
    def clean_duplicate_languages(self):
        seen_languages = set()
        for cl in self.character_languages.all():
            if cl.language.name in seen_languages:
                cl.delete()
            else:
                seen_languages.add(cl.language.name)
...
class Note(models.Model):
    character_sheet = models.ForeignKey(CharacterSheet, related_name='notes', on_delete=models.CASCADE)
