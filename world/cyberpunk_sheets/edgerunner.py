...
class EdgerunnerChargen:
    ...
    def create_character(cls, sheet, role, method, full_name):
        logger.info(f"Starting create_character for {full_name}, role: {role}, method: {method}")
        try:
            # Reset the character sheet
            cls.reset_character(sheet)
            
            # Set the role and full name
            sheet.role = role
            ...
            if method == "edgerunner":
                ...
                cls.initialize_hit_points(sheet)
                cls.assign_skills(sheet, role)
                cls.assign_gear(sheet, role)
                cls.assign_cyberware(sheet, role)
                sheet.eurodollars = 500  # Set starting money for Edgerunner method
                result = f"Character created using the Edgerunner method for role: {role}.\n"
                result += f"You have {sheet.attribute_points} stat points and {sheet.skill_points} skill points left to allocate.\n"
                result += f"{sheet.eurodollars} Eurodollars have been added to your account."
            elif method == "complete_package":
                result = cls.complete_package_chargen(sheet)
                sheet.role = role  # Ensure role is set for Complete Package method
            else:
                raise ValueError(f"Invalid character creation method: {method}")
            
            logger.info(f"Character creation completed for {full_name}")
            ...
            final_message = result + "\nUse 'sheet' to view your full character details and 'balance' to check your money."
    ...
    def complete_package_chargen(cls, sheet):
        ...
        # Initialize stats to 2
        for stat in ['intelligence', 'reflexes', 'dexterity', 'technology', 'cool', 'willpower', 'luck', 'move', 'body', 'empathy']:
            setattr(sheet, stat, 2)
            sheet.attribute_points -= 2

        cls.initialize_hit_points(sheet)  # Initialize hit points based on body stat
        ...
        sheet.save()
        return f"Character created using the Complete Package method. You have {sheet.attribute_points} attribute points and {sheet.skill_points} skill points to allocate."
    ...
    def assign_languages(cls, sheet, role):
        ...
        # List of default languages and their levels
        default_languages = [
            ("Streetslang", 4),
            ("English", 4)
        ]

        for lang_name, level in default_languages:
        ...
        # Get all languages except Streetslang
        available_languages = Language.objects.exclude(name__in=["Streetslang", "English"])
        ...
        if available_languages.exists():
            ...
            CharacterLanguage.objects.get_or_create(
                ...
                defaults={'level': random_level}
            )
        
        logger.info(f"Languages assigned: {', '.join(sheet.character_languages.all().values_list('language__name', flat=True))}")
        ...
        for weapon in inventory.weapons.all():
            ...
            if ammo:
                try:
                    ...
                    logger.error(f"Error adding ammunition {ammo['name']}: {str(e)}")
    ...
    @classmethod
    def initialize_hit_points(cls, sheet):
        body_value = getattr(sheet, 'body', 0)
        max_hp = 10 + (5 * body_value)
        sheet._max_hp = max_hp
        sheet._current_hp = max_hp
